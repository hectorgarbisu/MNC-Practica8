...
double eTimer::stop(){
	...
	average += last;
	number ++;
	return (double)(last - offset)/(double)frecuency;
}
...
void eTimer::report(){
	...
	printTime((double)(min-offset)/(double)frecuency);
	printTime((double)(average+number/2-number*offset)/((double)number*(double)frecuency));
	printTime((double)(max-offset)/(double)frecuency);
	printf("\n");
}

double eTimer::get(){

	return (double)(min-offset)/(double)frecuency;
}
